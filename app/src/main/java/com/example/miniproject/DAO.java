...
public class DAO {
    ...
    public ArrayList<ProductDTO> makeMenu(String name){
        ...
        if(name.equals("gimsT")){
            ...
            list.add(new ProductDTO("특공대라면","특별한 비법소스를 첨가한 기본 라면", 2500,10));
            list.add(new ProductDTO("떡라면","떡이 추가되었습니다~", 3000,8));
            list.add(new ProductDTO("치즈라면","치즈가 추가되었어요~", 3000,7));
            list.add(new ProductDTO("해물라면","특공대 라면에 갖은 해물이~", 4000,5));

        } else if(name.equals("noodlesT")){
            list.add(new ProductDTO("특공대국수","고소함을 느낄 수 있는 에스프레소", 2500,10));
            list.add(new ProductDTO("비빔국수","원두를 갈아만든 커피", 3000,10));
            list.add(new ProductDTO("쫄면","아메리카노와 우유가 만난 커피", 4000,10));
            list.add(new ProductDTO("특공대우동","우유와 바닐라가 조화를 이루는 음료", 4000,10));

        } else if(name.equals("ricesT")){
            list.add(new ProductDTO("특공대비빔밥","고소함을 느낄 수 있는 에스프레소", 2500,10));
            list.add(new ProductDTO("제육덮밥","원두를 갈아만든 커피", 3000,10));
            list.add(new ProductDTO("오징어덮밥","아메리카노와 우유가 만난 커피", 4000,10));
            list.add(new ProductDTO("김치볶음밥","우유와 바닐라가 조화를 이루는 음료", 4000,10));

        } else if(name.equals("tteoksT")){
            list.add(new ProductDTO("특공대떡볶이","고소함을 느낄 수 있는 에스프레소", 2500,10));
            list.add(new ProductDTO("치즈떡볶이","원두를 갈아만든 커피", 3000,10));
            list.add(new ProductDTO("라뽁이","아메리카노와 우유가 만난 커피", 4000,10));
            list.add(new ProductDTO("짜장떡볶이","우유와 바닐라가 조화를 이루는 음료", 4000,10));

        } else if(name.equals("frysT")){
            list.add(new ProductDTO("고구마튀김","고소함을 느낄 수 있는 에스프레소", 2500,10));
            list.add(new ProductDTO("고추튀김","원두를 갈아만든 커피", 3000,10));
            list.add(new ProductDTO("오징어튀김","아메리카노와 우유가 만난 커피", 4000,10));
            list.add(new ProductDTO("특공대튀김","우유와 바닐라가 조화를 이루는 음료", 4000,10));
